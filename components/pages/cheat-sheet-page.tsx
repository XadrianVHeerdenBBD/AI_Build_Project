...
// YouTube links for each section
...
export function CheatSheetPage({ onNext }: CheatSheetPageProps) {
  ...
  const handlePlayButtonClick = () => {
    // Redirect to the corresponding YouTube video for current section
          ...
          <div className="col-span-1">
            <Card className="p-4 bg-teal-700 text-white border-0 space-y-2">
              ...
              {cheatSheetSections.map((section, i) => (
                <button
                  key={section.label}
                  className={`w-full py-2 px-4 rounded font-semibold transition mb-6 ${
                    activeIndex === i
                      ? "bg-black text-white"
              ...
              {/* <a href={loops as string} download="loops.png" className="block mt-4"> */}
              <div className="pt-4 flex justify-center">
                <div className="w-8 h-8 text-white">↓</div>
              </div>
              {/* </a> */}
            </Card>
            {/* Play Button - shown for all sections, opens corresponding video */}
            <div className="flex flex-col items-center mt-8">
              ...
              >
                <div className="rounded-full border-2 border-teal-700 w-16 h-16 flex items-center justify-center text-teal-700 text-2xl font-bold hover:bg-teal-100">
                  ▶
                </div>
          ...
          <div className="col-span-3 space-y-6">
            ...
            {activeIndex === 1 && (
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-3 gap-4 mb-6">
                  {["for", "while", "do-while", "range-based", "nested", "break/continue"].map((type, idx) => (
                    <div
                      key={type}
                      className={`border-4 ${
                        idx === 0 ? "border-cyan-500 bg-white text-black" :
                        idx === 1 ? "border-green-500 bg-white text-black" :
                        idx === 2 ? "border-yellow-600 bg-white text-black" :
                        idx === 3 ? "border-red-500 bg-white text-black" :
                        idx === 4 ? "border-blue-700 bg-white text-black" :
                        "border-pink-600 bg-white text-black"
                      } rounded text-center p-1 font-semibold`}
                    >
                      {type}
                    </div>
                  ))}
                ...
                {loopExamples.map((example) => (
                  <Card key={example.type} className={`border-4 ${example.color} bg-white text-black`}>
                    <div className="p-6">
            ...
            {activeIndex === 2 && (
              <div className="space-y-6">
                {/* What it is */}
                <div>
                  <div className="px-4 py-2 rounded-t bg-blue-500 text-white font-bold">What it is</div>
                ...
                {/* Roles */}
                <div>
                  <div className="px-4 py-2 rounded-t bg-green-500 text-white font-bold">Roles</div>
                  <div className="bg-white border border-green-500 rounded-b px-6 py-4">
                    <ul className="list-disc pl-5">
                      {observerContent.roles.map((role, i) => <li key={i}>{role}</li>)}
                    </ul>
                  </div>
                </div>
                {/* Variants & options */}
                <div>
                  <div className="px-4 py-2 rounded-t bg-yellow-500 text-white font-bold">Variants & options</div>
                  <div className="bg-white border border-yellow-500 rounded-b px-6 py-4">
                    <ul className="list-disc pl-5">
                      {observerContent.variants.map((variant, i) => <li key={i}>{variant}</li>)}
                    </ul>
                  </div>
                </div>
                {/* When to use */}
                <div>
                  <div className="px-4 py-2 rounded-t bg-purple-500 text-white font-bold">When to use</div>
                  <div className="bg-white border border-purple-500 rounded-b px-6 py-4">
                    <ul className="list-disc pl-5">
                      {observerContent.when.map((when, i) => <li key={i}>{when}</li>)}
                    </ul>
                  </div>
                </div>
                {/* UML */}
                <div>
                  <div className="px-4 py-2 rounded-t bg-pink-500 text-white font-bold">Minimal UML (text)</div>
                ...
                {/* Key guidelines */}
                <div>
                  <div className="px-4 py-2 rounded-t bg-red-500 text-white font-bold">Key guidelines</div>
                  <div className="bg-white border border-red-500 rounded-b px-6 py-4">
                    <ul className="list-disc pl-5">
                      {observerContent.key.map((key, i) => <li key={i}>{key}</li>)}
                    </ul>
                  </div>
                </div>
              </div>
