      ...
      async queryFn(arg) {
        ...
        try {
          ...
          if (patternId) {
            ...
            studentIdsForPattern = profileData?.map((p: { student_id: string }) => p.student_id) ?? [];
          }
          ...
          if (patternId) {
            quizQuery = quizQuery.in("student_id", allStudents?.map((s: { id: string }) => s.id) ?? []);
          }
          ...
          type AttemptItem = { is_correct: boolean; points_earned: number | null };
          const processedAttempts = (quizAttempts ?? []).map((attempt: { student_id: string; quiz_type: { quiz_type: string } | null; quiz_attempt_item: AttemptItem[] | null }) => {
            ...
            const correctAnswers = attempt.quiz_attempt_item?.reduce(
              (sum: number, item: AttemptItem) => sum + (item.is_correct ? 1 : 0),
              0
            );
          ...
          type ProcessedAttempt = { student_id: string; type: string; totalQuestions: number; correctAnswers: number | undefined };
          const finalByStudent: Record<string, ProcessedAttempt> = {};
          ...
          const avgScore =
            Object.values(finalByStudent).length > 0
              ? Object.values(finalByStudent)
                  .map((a) => (a.totalQuestions > 0 ? ((a.correctAnswers ?? 0) / a.totalQuestions) * 100 : 0))
